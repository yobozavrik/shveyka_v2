...
function SimpleQuantityOperationCard({
  operation,
  entries,
  stageCode,
  saving,
  ...
  return (
    <section className="rounded-[28px] border border-[var(--border)] bg-[var(--bg-card)] p-5 space-y-4">
      <div className="flex items-center gap-3">
        <div className={clsx('rounded-2xl p-3', stageConfig.bgAccent, stageConfig.textAccent)}>
...
function PackagingOperationCard({
  operation,
  entries,
  stageCode,
  saving,
...
function DynamicFormOperationCard({
  operation,
  entries,
  draft,
  setDraft,
  ...
  const buttonColorClass = clsx(
    'flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-4 text-sm font-black text-white disabled:opacity-60',
    stageConfig.color === 'emerald' && 'bg-emerald-500',
    stageConfig.color === 'blue' && 'bg-blue-500',
    stageConfig.color === 'purple' && 'bg-purple-500',
    stageConfig.color === 'indigo' && 'bg-indigo-500',
    stageConfig.color === 'violet' && 'bg-violet-500',
    stageConfig.color === 'orange' && 'bg-orange-500',
  );
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 px-4 py-4 backdrop-blur-sm sm:items-center">
      <div className="w-full max-w-md rounded-[28px] border border-[var(--border)] bg-[var(--bg-card)] p-5 shadow-2xl">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          ...
          >
            {busy ? 'Збереження...' : (confirmButtonText || 'Так, завершити')}
          </button>
        </div>
      </div>
    </div>
...
export default function TaskPage() {
  ...
  // Для раскроя показываем ТОЛЬКО операцию nastil (убираем дублирующую cutting)
  ...
  const validateBeforeComplete = () => {
    ...
    if (entries.length === 0) {
      setValidationErrors([{
        field: 'entries',
        label: 'Немає жодного запису. Додайте хоча б один запис перед завершенням.',
        operation: '',
      }]);
      return false;
    }

    // Для каждой операции проверяем что есть записи
    operations.forEach((operation) => {
      ...
      // Если у операции нет ни одного записи - это ошибка
      if (operationEntries.length === 0) {
        errors.push({
          field: operation.id.toString(),
          label: `Операція "${operation.name}" не має жодного запису. Додайте запис перед завершенням.`,
  ...
  const handleCompleteClick = () => {
    ...
    if (!isValid) {
      // Показываем модалку с ошибками
      return;
    }

    // Всё валидно - открываем подтверждение
    setConfirmOpen(true);
  };
  ...
  return (
    <div className="px-4 py-5 pb-24 space-y-5">
        ...
        <section className="space-y-4">
          ...
          {isEmbroideryStage ? (
            ...
            operations.map((operation) => {
              ...
              // Simple quantity stages (sewing, overlock, straight_stitch, coverlock)
          ...
          >
            {busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <CheckCircle2 className="h-5 w-5" />}
            {stageConfig.completeButtonText || 'Завершити етап'}
          </button>
        </section>
      )}
      ...
      {validationErrors.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 px-4 py-4 backdrop-blur-sm sm:items-center">
          <div className="w-full max-w-md rounded-[28px] border border-red-200 bg-[var(--bg-card)] p-5 shadow-2xl">
            <div className="flex items-start gap-3">
              <div className="rounded-2xl bg-red-500/15 p-3 text-red-500">
                <AlertTriangle className="h-5 w-5" />
              </div>
              <div className="min-w-0 flex-1">
                <div className="text-lg font-black text-[var(--text-1)]">Неможливо завершити етап</div>
                <div className="mt-2 space-y-2">
                  {validationErrors.map((error, index) => (
                    <div
                      key={index}
                      className="rounded-xl bg-red-500/10 border border-red-200 p-3 text-sm"
                    >
                      <div className="font-bold text-red-600">{error.label}</div>
                    </div>
                  ))}
                </div>
                <div className="mt-4 text-xs text-[var(--text-3)]">
                  Будь ласка, заповніть всі необхідні поля та додайте записи перед завершенням.
                </div>
              </div>
            </div>
            ...
            <div className="mt-5">
              <button
                ...
                className="w-full rounded-2xl border border-[var(--border)] bg-[var(--bg-base)] px-4 py-3 text-sm font-black text-[var(--text-2)]"
              >
                Зрозуміло
              </button>
            </div>

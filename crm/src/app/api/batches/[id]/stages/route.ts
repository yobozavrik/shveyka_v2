...
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    if (!Number.isFinite(batchId)) {
      return ApiResponse.error('Invalid batch id', ERROR_CODES.BAD_REQUEST, 400);
    }
    ...
    if (stagesError) {
      return ApiResponse.handle(stagesError, 'batches_id_stages_get');
    }

    if (tasksError) {
      return ApiResponse.handle(tasksError, 'batches_id_stages_get');
    }

    if (entriesError) {
      return ApiResponse.handle(entriesError, 'batches_id_stages_get');
    }

    if (operationsError) {
      return ApiResponse.handle(operationsError, 'batches_id_stages_get');
    }
    ...
    return ApiResponse.handle(error, 'batches_id_stages_get');
  }
}
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    if (action !== 'advance') {
      return ApiResponse.error('Unsupported action', ERROR_CODES.BAD_REQUEST, 400);
    }
    ...
    if (stagesError) {
      return ApiResponse.handle(stagesError, 'batches_id_stages_post');
    }

    const orderedStages = (stages || []) as StageRow[];
    const currentStage = orderedStages.find((stage) => stage.code === batch.status) || null;
    if (!currentStage) {
      return ApiResponse.error('Партія ще не запущена в етап', ERROR_CODES.BAD_REQUEST, 400);
    }

    const { data: tasks, error: tasksError } = await loadTasks(batchId);
    if (tasksError) {
      return ApiResponse.handle(tasksError, 'batches_id_stages_post');
    }

    const currentTask = groupLatestTask((tasks || []) as TaskRow[], currentStage.id);
    if (!currentTask || currentTask.status !== 'completed') {
      return ApiResponse.error('Поточний етап ще не завершено', ERROR_CODES.BAD_REQUEST, 400);
    }
    ...
    if (!nextStage) {
      return ApiResponse.error('Це останній етап партії', ERROR_CODES.BAD_REQUEST, 400);
    }

    const activeNextTask = (tasks || []).find((task) => task.stage_id === nextStage.id && ['pending', 'accepted', 'in_progress'].includes(task.status));
    if (activeNextTask) {
      return ApiResponse.error('Наступний етап уже створено або запущено', ERROR_CODES.BAD_REQUEST, 400);
    }
    ...
    if (insertError || !nextTask) {
      return ApiResponse.handle(insertError || new Error('Не вдалося створити завдання наступного етапу'), 'batches_id_stages_post');
    }
    ...
    if (updateError || !updatedBatch) {
      ...
      return ApiResponse.handle(updateError || new Error('Не вдалося оновити статус партії'), 'batches_id_stages_post');
    }
    ...
    return ApiResponse.handle(error, 'batches_id_stages_post');
  }
}

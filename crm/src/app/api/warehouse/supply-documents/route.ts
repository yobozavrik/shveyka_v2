...
export async function GET(request: Request) {
  try {
    ...
    if (error) return ApiResponse.handle(error, 'warehouse_supply_documents');
    return ApiResponse.success(data || []);
  } catch (error) {
    return ApiResponse.handle(error, 'warehouse_supply_documents');
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (docErr) return ApiResponse.handle(docErr, 'warehouse_supply_documents');
    ...
    if (items.length > 0) {
      ...
      if (itemsErr) return ApiResponse.handle(itemsErr, 'warehouse_supply_documents');
    }

    return ApiResponse.success(doc, 201);
  } catch (error) {
    return ApiResponse.handle(error, 'warehouse_supply_documents');
  }
}
